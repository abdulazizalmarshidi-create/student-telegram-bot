...
async def notify_admin(context: ContextTypes.DEFAULT_TYPE, ticket_code: str):
    ...
    text = (
        ...
        "💬 ويمكنك أيضاً التواصل معه من زر «محادثة» بالأسفل."
    )
...
async def confirm_ticket(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        ...
        "الحالة: 🆕 جديدة\n\n"
        "📌 لضمان سرعة معالجة طلبك:\n"
        f"نرجو إرسال رقم التذكرة وتفاصيل المشكلة بالخاص على تيليجرام إلى @{CONTACT_USERNAME}.\n\n"
        "💬 ويمكنك أيضاً متابعة التذكرة والتواصل مع الدعم من داخل البوت.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("👤 التواصل بالخاص", url=f"https://t.me/{CONTACT_USERNAME}")],
            [InlineKeyboardButton("💬 التواصل داخل البوت", callback_data=f"student_chat:{ticket_code}")],
...
async def track_ticket_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        if ticket["telegram_user_id"] == update.effective_user.id
